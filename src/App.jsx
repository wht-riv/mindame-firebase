...
export default function App(){
 ...
 const[profile,setProfile]=useState(null),[loading,setLoading]=useState(false),[records,setRecords]=useState([]),[posts,setPosts]=useState([]),[tab,setTab]=useState('home'),[flow,setFlow]=useState(null),[toast,setToast]=useState('');
...
if (!DEMO_MODE) {
  ...
  await loadAll();setFlow(null);setTab('home');msg('今日の作戦を記録しました。')
 }
...
function Shell({children,profile,toast}){return <div className="shell"><header><b>みんだめ<span>v4</span></b></header><main>{children}</main>{toast&&<div className="toast">{toast}</div>}</div>}
