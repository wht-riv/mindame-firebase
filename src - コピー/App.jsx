...
const STORAGE_CODE = 'mindameParticipantCodeV3';
...
function App() {
  ...
  const participantRef = useMemo(() => uid && db ? doc(db, 'participantsV3', uid) : null, [uid]);
  ...
  useEffect(() => {
    ...
    ensureAnonymousUser().then(user => {
      if (!user) { setScreen('config'); return; }
      setUid(user.uid);
      if (!participantCode) setScreen('entry');
    }).catch(e => { console.error(e); setScreen('config'); });
  }, []);

  useEffect(() => {
    if (uid && participantCode) loadParticipant();
  }, [uid, participantCode]);

  async function loadParticipant() {
    setScreen('loading');
    try {
      const ref = doc(db, 'participantsV3', uid);
  ...
  async function createPlan(plan) {
    ...
    if (plan.shareExcuse) {
      await addDoc(collection(db, 'publicPostsV3'), {
  ...
  async function submitReview(result) {
    const recordRef = doc(db, 'participantsV3', uid, 'records', pending.id);
  ...
  async function loadPublicPosts() {
    setScreen('community');
    try {
      const qy = query(collection(db, 'publicPostsV3'), where('isVisible', '==', true), limit(30));
      const snap = await getDocs(qy);
      setPublicPosts(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)));
    } catch (e) {
  ...
  async function react(post, key) {
    await setDoc(doc(db, 'publicPostsV3', post.id, 'reactions', uid), {
      ownerUid: uid, type: key, createdAt: serverTimestamp()
    });
    notify(key === 'relate' ? '「わかる」を送りました。' : 'リアクションを送りました。');
  }

  const report = useMemo(() => {
    const acted = history.filter(r => r.outcomeStatus === 'acted').length;
    const matched = history.filter(r => r.outcomeStatus === 'not_done' && r.predictionMatched === 'yes').length;
    const discovered = history.filter(r => r.outcomeStatus === 'not_done' && r.predictionMatched === 'no').length;
    const types = new Set(history.map(r => r.predictedExcuse).filter(Boolean)).size;
    return { acted, matched, discovered, types };
  }, [history]);

  const pace = useMemo(() => {
    const valid = history.filter(r => ['acted', 'not_done'].includes(r.outcomeStatus)).slice(0, 7);
  ...
  return <Shell profile={profile} logout={logout} toast={toast}><Home profile={profile} pending={pending} pace={pace} report={report} history={history} onPlan={() => setScreen('plan')} onCommunity={loadPublicPosts} /></Shell>;
}
...
function Home({ profile, pace, report, history, onPlan, onCommunity }) {
...
function PredictionReport({report}) {
  return <div className="card"><h2>言い訳予測レポート</h2><p className="muted">高いほど良いランキングではなく、最近の記録の内訳です。</p><div className="report-grid"><div><b>{report.acted}</b><span>予測があっても動けた</span></div><div><b>{report.matched}</b><span>予測と理由が一致</span></div><div><b>{report.discovered}</b><span>新しい理由を発見</span></div><div><b>{report.types}</b><span>使った言い訳の種類</span></div></div></div>;
}

function Pace({ pace }) {
  if (pace.valid < 4) return <div className="card"><h2>最近のペース</h2><p>記録をためている途中です。</p><p className="muted">数回の振り返りが集まると、実行と未実行の配分が見えるようになります。</p></div>;
  const label = pace.percent < 50 ? '休息・未実行多め' : pace.percent <= 70 ? 'みんだめ上の「ちょうどいい」' : '行動多め';
  return <div className="card pace-card"><div className="pace-head"><div><h2>最近のペース</h2><p>{label}</p></div><strong>{pace.percent}%</strong></div><div className="meter"><div className="zone low"/><div className="zone middle"/><div className="zone high"/><i style={{left:`calc(${pace.percent}% - 7px)`}} /></div><div className="meter-labels"><span>休息・未実行多め</span><span>ちょうどいい</span><span>行動多め</span></div><div className="breakdown"><span>実行 <b>{pace.acted}</b></span><span>未実行 <b>{pace.notDone}</b></span></div><small>直近{pace.valid}回の有効な振り返りから計算しています。50～70％は科学的な最適値ではなく、100％だけを理想にしないための設計上の目安です。</small></div>;
}
...
function Reflection({ record, onNext, onHome }) {
  ...
  if(record.outcomeStatus==='acted'){title=record.continuedBeyondFirstStep?'最初の一歩から、その先まで進めました。':'最初の一歩を実行できました。';message=record.continuedBeyondFirstStep?'「意外とできた」が生まれた日です。':`「${record.predictedExcuse}」と予測した日にも、${record.firstStepText}ができました。`;}
  if(record.outcomeStatus==='rested'){title='休息を選んだ記録です。';message='休んだことも、最近のペースを考えるための記録です。';}
  if(record.outcomeStatus==='not_done' && record.predictionMatched==='yes'){title='予測的中。';message='自分が止まりやすい状況を、事前に捉えられていたようです。';}
  if(record.outcomeStatus==='not_done' && record.predictionMatched==='no'){title='予想外の理由でした。';message='そんな日もあります。次の作戦に使える新しい発見です。';}
